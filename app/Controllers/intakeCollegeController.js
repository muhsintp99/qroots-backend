const IntakeCollege = require('../models/intakeCollege');
...
const createIntake = async (req, res) => {
  try {
    ...
    const populatedIntake = await IntakeCollege.findById(saved._id).populate({
      path: 'college',
...
const getAllIntakes = async (req, res) => {
  try {
    const intakes = await IntakeCollege.find({ isDeleted: false })
      .populate({
        path: 'college',
        match: { isDeleted: false },
        select: '-__v -createdBy -updatedBy -isDeleted -createdAt -updatedAt'
...
const hardDeleteIntake = async (req, res) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
      return res.status(400).json({
        success: false,
...
const getIntakeCount = async (req, res) => {
  try {
